n = int(input("Entrez le nombre de pays que visiteront Joseph et Haruhi."))
...
for i in range(0, n):
    try: (id, decalage) = list(map(int, input("Entrez l'ID du pays et son décalage.").split()))
    except ValueError:
...
v = int(input("Entrez le nombre de voyages que feront Joseph et Haruhi."))
if v < 1 or v > 1000:
...
liste_e = [None] * v
for j in range(0, v):
    try: (voyageur, destination) = list(map(int, input("Entrez le numéro du voyageur et l'ID du pays de destination.").split()))
    except ValueError:
        print("Contrainte: Le format n'est pas respecté")
