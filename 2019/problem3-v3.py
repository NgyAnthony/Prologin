...
jours = list(map(int, "4 1 7 9 7".split())) # Input days where friends are present
...
def secondFilter(all_items_list, individual_list):
    ...
    for f in range(0, len(all_items_list)): # This loop is going to repeat until all positions are tried
        ...
        if difference_sum == 0 or difference_sum > m:
            total_friends = 0
        ...
        print(final_friends_amount)

    if m < max(friends_in_individual_list) or m < max(final_friends_amount):
        print(m)
    elif max(friends_in_individual_list) > max(final_friends_amount): # If a number in individual list is larger among processed ones, print 'individual'
...
#Tomorrow morning :
#Use the debugger to find where does the algorithm fails